...
log "applying strict Traefik routing for subpath + IDP..."
kctl -n "${NAMESPACE}" apply -f - <<EOF
apiVersion: traefik.io/v1alpha1
kind: Middleware
metadata:
  name: ${RELEASE_NAME}-strip-subpath-prefix
spec:
  stripPrefix:
    prefixes:
      - ${SUBPATH}
...
kind: Middleware
metadata:
  name: ${RELEASE_NAME}-drop-auth-header
spec:
  headers:
...
kind: IngressRoute
metadata:
  name: ${RELEASE_NAME}-ocis-wellknown
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(\`${INGRESS_HOST}\`) && PathPrefix(\`${SUBPATH}/.well-known\`)
      kind: Rule
      services:
        - name: ${RELEASE_NAME}-idp
          port: 9130
  tls: {}
---
apiVersion: traefik.io/v1alpha1
kind: IngressRoute
metadata:
  name: ${RELEASE_NAME}-ocis-subpath-idp
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(\`${INGRESS_HOST}\`) && (PathPrefix(\`${SUBPATH}/signin\`) || PathPrefix(\`${SUBPATH}/konnect\`))
      kind: Rule
      services:
        - name: ${RELEASE_NAME}-idp
          port: 9130
...
kind: IngressRoute
metadata:
  name: ${RELEASE_NAME}-ocis-subpath-capabilities
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(\`${INGRESS_HOST}\`) && Path(\`${SUBPATH}/ocs/v2.php/cloud/capabilities\`)
      kind: Rule
      middlewares:
        - name: ${RELEASE_NAME}-strip-subpath-prefix
        - name: ${RELEASE_NAME}-drop-auth-header
...
kind: IngressRoute
metadata:
  name: ${RELEASE_NAME}-ocis-subpath-api
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(\`${INGRESS_HOST}\`) && (PathPrefix(\`${SUBPATH}/graph\`) || PathPrefix(\`${SUBPATH}/api\`) || PathPrefix(\`${SUBPATH}/dav\`) || PathPrefix(\`${SUBPATH}/data\`) || PathPrefix(\`${SUBPATH}/remote.php\`) || PathPrefix(\`${SUBPATH}/ocs\`) || PathPrefix(\`${SUBPATH}/thumbnails\`))
      kind: Rule
      middlewares:
        - name: ${RELEASE_NAME}-strip-subpath-prefix
      services:
...
kind: IngressRoute
metadata:
  name: ${RELEASE_NAME}-ocis-web
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(\`${INGRESS_HOST}\`) && PathPrefix(\`${SUBPATH}\`)
      kind: Rule
      services:
        - name: ${RELEASE_NAME}
          port: 9200
