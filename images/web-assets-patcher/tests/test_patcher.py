...
class PatcherTests(unittest.TestCase):
    ...
    def test_patch_markdown_image_sources_loads_relative_images_via_webdav_blob(self):
        ...
        self.assertIn("__ocisPublicMarkdownRoute=()=>", patched)
        self.assertIn("__ocisPublicMarkdownWebDavPath=()=>", patched)
        self.assertIn('/(?:text-editor|preview)\\/public\\/([^/]+)(?:\\/(.*))?$/', patched)
        self.assertIn("webDavPath:`/public-files/${encodeURIComponent(r)}", patched)
        self.assertIn("__ocisPublicMarkdownWebDavPath()||t.resource?.webDavPath", patched)
        self.assertIn("__ocisMarkdownImageRequestHeaders=()=>", patched)
        self.assertIn("sessionStorage.getItem(`oc.publicLink.${t}.password`)", patched)
        self.assertIn('{"public-token":t.token}', patched)
        self.assertIn('Authorization=`Basic ${btoa(unescape(encodeURIComponent(`public:${n}`)))}`', patched)
        ...
        self.assertIn("headers:r", patched)
        self.assertIn("Authorization:`Bearer ${e}`", patched)
